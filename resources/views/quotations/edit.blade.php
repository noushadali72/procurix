                ...
                $('#vendorForm').on('submit', function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            showToast('success', response.message || 'Vendor created successfully.');
                            closeVendorModal();
                        ...
                        error: function(xhr) {
                            ...
                            showToast('error', xhr.responseJSON?.message ||'Unable to create vendor.');
                        },
            ...
            $(document).ready(function() {
                $('#quotationForm').on(
                    ...
                    function(e) {
                        ...
                        $.ajax({
                            url: form.attr('action'),
                            // POST because @method('PUT')
                            // provides Laravel method spoofing.
                            type: 'POST',
                            data: form.serialize(),
                            headers: {
                                'Accept': 'application/json'
                            ...
                            error: function(xhr) {
                                button.prop('disabled', false);
                                buttonText.text('Update Quotation');

                                if (xhr.status === 422) {
                                    showErrors(xhr.responseJSON.errors);
                                    return;
                                }
                                showToast('error', xhr.responseJSON?.message ??'Unable to update quotation.');
                            }
                        });
                    }
                );
                ...
                function showErrors(errors) {
                        ...
                        // Dynamic error handling for items
